...
echo "Running test $1 ..."
OUTPUT=$(PYTHONPATH=${PYTHONPATH} QGIS_TEST_MODULE=${TEST_NAME} unbuffer /usr/bin/qgis_testrunner.py "$TEST_NAME"  2>/dev/null | tee /dev/tty)
#qgis_testrunner.py test_suite.testpackage
EXIT_CODE="$?"
if [ -z "$OUTPUT" ]; then
